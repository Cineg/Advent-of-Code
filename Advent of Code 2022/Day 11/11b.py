...
class Monkey:
    ...
    def monkey_business(self, modulo: int) -> list[tuple[int, int]]:
        throws: list[tuple] = []
        while self.items:
...
def main() -> None:
    ...
    max_divisor: int = 1
    for index in input:
        max_divisor *= input[index].division_test

    i: int = 0
    while i < 10000:
        for index in input:
            monkey: Monkey = input[index]
            throw: list[tuple[int, int]] = monkey.monkey_business(max_divisor)
            for item in throw:
